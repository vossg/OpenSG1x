    ...
    if(getTextEnabled())
        {
            ...
            glPushMatrix();
              ...
              drawString(base, real2String(value), STATISTICS_CENTER, STATISTICS_TOP);        
            glPopMatrix();
...
void GraphicStatisticsForeground::drawChart(UInt32 ID,   
                                            StatElem* el,
                                            DrawActionBase* base, 
                                            Viewport* port)
{
    ...
    /* The collor of the chart is set by the currentColor of this
       statistics Element */
    glColor4f(getColorCurrent()[ID][0], getColorCurrent()[ID][1], getColorCurrent()[ID][2],getColorCurrent()[ID][3]);
    ...
    if(getTextEnabled()){
		...
		glPushMatrix();
          ...
          drawString( base , valstr , STATISTICS_CENTER);
		glPopMatrix();
...
void GraphicStatisticsForeground::drawBar(UInt32 ID,   
                                          StatElem* el,
                                          DrawActionBase* base, 
                                          Viewport* port)
{
  ...
  //cout << "Bar value: " << value << endl;
    ...
    if (xdist < 0.5)
    ...
    else //if xdist > 0.5
        {
            ...
            // draw first quad
              ...
              glVertex2f(textWidth, textHeight);
              glVertex2f(textWidth, 1.0);
              glColor4f(CurrentColor[0],CurrentColor[1],CurrentColor[2], getColorCurrent()[ID][3]);
              glVertex2f(0.5, 1.0);
              glVertex2f(0.5, textHeight);
            // draw second quad
              glVertex2f(0.5, textHeight);
              glVertex2f(0.5, 1.0);
              glColor4f(MaxColor[0]*((xdist-0.5)*2)+CurrentColor[0]*(1-((xdist-0.5)*2)),
                        MaxColor[1]*((xdist-0.5)*2)+CurrentColor[1]*(1-((xdist-0.5)*2)),
                        MaxColor[2]*((xdist-0.5)*2)+CurrentColor[2]*(1-((xdist-0.5)*2)), 
                        getColorMax()[ID][3]);
    ...
    if(getTextEnabled()){
        ...
        glPushMatrix();
          ...
          glScalef(0.15*ratio, 0.15, 1.0);
          drawString(base, maxstr, STATISTICS_RIGHT);
        glPopMatrix();
...
void GraphicStatisticsForeground::drawLineChart(UInt32 ID,   
                                                StatElem* el,
                                                DrawActionBase* base, 
                                                Viewport* port)
{ 
...
void GraphicStatisticsForeground::drawText(UInt32 ID,   
                                             StatElem* el,
                                             DrawActionBase* base, 
                                             Viewport* port)
{
                                                 ...
                                                 Vec2f* Size)
{
    ...
    */
    ...
    if( size[0] > 1.0 )   size[0]  =  size[0] / pw;
    if( size[1] > 1.0 )   size[1]  =  size[1] / ph;
    ...
    */
    ...
    if( osgabs(pos[0]) > 1.0 )   pos[0]  =  pos[0] / pw;
    if( osgabs(pos[1]) > 1.0 )   pos[1]  =  pos[1] / ph;
  
  
    // check whether the positions are relative to the right/bottom
    if( xneg )   pos[0]  =  1.0 + pos[0] - size[0];
    if( yneg )   pos[1]  =  1.0 + pos[1] - size[1];
                                             ...
                                             )
{
    ...
    if(align == STATISTICS_RIGHT)  deltaX = -(positions[n-3][0]);
    if(align == STATISTICS_CENTER) deltaX = -(positions[n-3][0]) / 2.0;
 
    if(valign != STATISTICS_BOTTOM)
        {
            Real32 textHeight=0.0;
            Real32 foo = 0.0;
            ...
            if(valign == STATISTICS_TOP)    deltaY = -textHeight;
            if(valign == STATISTICS_MIDDLE) deltaY = -(textHeight/2.0);
