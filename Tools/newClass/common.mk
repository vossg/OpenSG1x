...
#	GCCHOME = /usr/local
  ...
  GCFLAGS = -c -o $@ -D_REENTRANT -D$(OS)_OS -Wall -frtti -ansi -Wno-unknown-pragmas -I. -D __EXTENSIONS__
  ifeq ($(TEMPDB),YES)
...
# NETLIB  = -lsocket -lxnet
  ...
  COMLIB  = -L$(PPOOL)/lib/$(OS) -lcompress$(LIBSUF)
	...
	S3LIB = -I$(PPOOL)/lib/$(OS) -ls3$(LIBSUF)
...
#	GCXXINC	= -I$(GCCHOME)/lib/g++-include
  ...
  ifeq ($(TEMPDB),YES)
	...
	NCFLAGS = -LANG:std -ptused -c -o $@ -D$(OS)_OS -DNATIVE_CC -DSTL_HSUFFIX -n32 -woff 3303,1355
  ...
  COMLIB  = -L$(PPOOL)/lib/$(OS)/$(LIBDIR) -lcompress$(LIBSUF)
	...
	QTINC		=	-I$(PPOOL)/include/qt
	QTLIB		= -L$(PPOOL)/lib/$(OS)/$(LIBDIR) -lqt
	CAMINC  = -I/usr/include -DCAMERA_LIB
...
ifeq ($(OS),Linux)
	GCC 			= gcc
	GCXX 		= g++
  GCFLAGS = -c -o $@ -D$(OS)_OS -Wall -DGNU_CC # -pedantic 
  ifeq ($(TEMPDB),YES)
	  GCFLAGS += -fno-implicit-templates
  endif
	GLD			= gcc

	NCC 		= /opt/egcs/bin/gcc
	NCXX 		= /opt/egcs/bin/g++
  NCFLAGS = -c -o $@ -D$(OS)_OS -Wall -pedantic -DEGCS_CC
	NLD			= /opt/egcs/bin/g++
  ...
  COMLIB  = -L$(PPOOL)/lib/$(OS) -lcompress$(LIBSUF)
	...
	ABLIB 	= -L$(PPOOL)/lib/$(OS)/$(LIBDIR) -labase$(LIBSUF)
	TIINC   = -I$(PPOOL)/include/texImage -D TEXIMAGE_LIB 
	TILIB   = -R$(PPOOL)/lib/$(OS)/$(LIBDIR) -ltexImage$(LIBSUF)
	GSINC   = -I$(PPOOL)/include/geoShape -D GEOSHAPE_LIB 
	GSLIB   = -R$(PPOOL)/lib/$(OS)/$(LIBDIR) -lgeoShape$(LIBSUF)
	ACFINC  = -I$(PPOOL)/include/appConfig -D APPCONFIG_LIB
	ACFLIB  = -L$(PPOOL)/lib/$(OS) -lappConfig$(LIBSUF)
	GLUTINC = -L$(PPOOL)/include -D GLUT_LIB
...
ifeq ($(findstring NT,$(OS)),NT)
  ...
  GCFLAGS = -c -o $@ -D$(OS)_OS -Wall -pedantic -ID:\include -DGNU_CC	
	...
	NCC 		= cl
	NCXX 		= cl
	NCFLAGS	= /c /Fo"$@" /D "ANSI_STL" /D "NT_OS" /D "NATIVE_CC" \
...
#	STLLIB  = std-2.0.1-vc5.0.lib std-2.0.1-vc5.0-mt.lib std-2.0.1-vc5.0-uss.lib std-2.0.1-vc5.0-mt-uss.lib 
  ...
  COMLIB  = compress.lib
	...
	DXLIB   = /libpath:"V:/lib/NT/directX" dsound.lib dxguid.lib
