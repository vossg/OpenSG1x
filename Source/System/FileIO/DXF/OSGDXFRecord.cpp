 ...
 */
std::istream &DXFRecord::getLine(std::istream &is, std::string &str, 
								 Char8 **lineend)
{
	str.erase();
...
Int8 DXFRecord::fetch(void)
{
	...
	{
		_isUnget = false;
		// CAUTION: a DXFRecord with _valueType == DXFTypeEOF may have an invalid
		// _groupCode, thus you must NOT access _valueTypeMap for it!
	...
	{
		std::cout << _inStreamLineNumber << ":'" << _records[_curRecord]._valueStr << "'" << std::endl;
	}
	...
	_records[_curRecord]._groupCode = atoi(_records[_curRecord]._valueStr.c_str());
