...
bool createdFunction(FieldContainerPtr& fcp,
                     RemoteAspect *)
{
    if(Node::getClassType() == fcp->getType()) nodes++;
    if(Transform::getClassType() == fcp->getType()) transforms++;
    if(Geometry::getClassType() == fcp->getType()) geometries++;
 ...
 */
bool destroyedFunction(FieldContainerPtr& fcp,
                       RemoteAspect *)
{
    if(Node::getClassType() == fcp->getType()) nodes--;
    if(Transform::getClassType() == fcp->getType()) transforms--;
 ...
 */
bool changedFunction(FieldContainerPtr& fcp,
                     RemoteAspect *)
{
    if(verbose)
        printf ( "Changed:%s %d\n",
...
void server()
{
    ...
    RemoteAspect::Functor changed=osgTypedFunctionFunctor2CPtrRef
        <bool,
        FieldContainerPtr,
        RemoteAspect     *>(changedFunction);

    RemoteAspect::Functor destroyed=osgTypedFunctionFunctor2CPtrRef
        <bool,
        FieldContainerPtr,
        RemoteAspect     *>(destroyedFunction);

    RemoteAspect::Functor created=osgTypedFunctionFunctor2CPtrRef
        <bool,
        FieldContainerPtr,
        RemoteAspect     *>(createdFunction);
