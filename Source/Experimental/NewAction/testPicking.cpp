...
// This is a quick hack that makes the tutorial use the newaction stuff.
...
// The IntersectActor, needed to set test rays.
IntersectActor  *pIntActor;
IntersectAction *pIntAction;

// The Action, neede form traversal
NewAction *pAction;
    ...
    switch(k)
    {
        ...
        case ' ':   // send a ray through the clicked pixel
                ...
                */
                ...
                pIntAction->setLine(l);
                
                tStart = getSystemTime();
                pAction->apply(fileroot);
                tStop  = getSystemTime();

                std::cerr << "NewAction tStart: " << tStart
                          << " tStop: "           << tStop
                          << " tDelta: "          << tStop - tStart
                          << std::endl;

                tStart = getSystemTime();
                pIntAction->apply(fileroot);
                tStop  = getSystemTime();

                std::cerr << "Action    tStart: " << tStart
                          << " tStop: "           << tStop
                          << " tDelta: "          << tStop - tStart
                          << std::endl;
                ...
                if(pIntActor->getHit() == true)
                {
                    ...
                    std::cerr << " object " << pIntActor->getHitObject  () 
                              << " tri "    << pIntActor->getHitTriangle() 
                              << " dist "   << pIntActor->getHitDist    ()
                              << std::endl;
                    
                    mgr->setHighlight(pIntActor->getHitObject());
                    
                    // stop the ray on the hit surface
                    isectPoints->setValue(l.getPosition() + 
                            l.getDirection() * pIntActor->getHitDist(), 1);
                    
                    // find the triangle that was hit
                    TriangleIterator it(pIntActor->getHitObject());
                    it.seek(pIntActor->getHitTriangle());
                    
                    // calculate its vertex positions in world space
                    Matrix m;
                    pIntActor->getHitObject()->getToWorld(m);
                ...
                if(pIntAction->didHit())
                {
                    std::cerr << " object " << pIntAction->getHitObject  ()
                              << " tri "    << pIntAction->getHitTriangle()
                              << " dist "   << pIntAction->getHitT       ()
                              << std::endl;
                }
...
int main(int argc, char **argv)
{
    ...
    // Create the Action, Iterator and IntersectActor.
    ...
    pAction    = NewAction      ::create();
    pIntActor  = IntersectActor ::create();
    pIntAction = IntersectAction::create();

    pAction->setIterator(PriorityIterator::create());
