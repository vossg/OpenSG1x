                          ...
                          const Char8 */*szInFieldname*/)
{
    ...
    StringLink szFromName (szOutNodename);
    StringLink szToName   (szInNodename );

    string        szFromFName(szOutFieldname);

    NameNodeMap::iterator mapIt = _pNameNodeMap->find(szFromName);
    ...
    if(mapIt != _pNameNodeMap->end())
    {
        ...
        DataElementDesc *pDesc = 
            pFrom->getReflexType().findElementDesc(szOutFieldname);


        if(pDesc != NULL)
        {
            uiFromId = pDesc->getElementId();
            uiValid++;
        }
        else
        {
            UInt32 uiStrLen = strlen(szOutFieldname);

            if(uiStrLen                  <=   8 ||
               szFromFName[uiStrLen - 8] != '_' ||
               szFromFName[uiStrLen - 7] != 'c' ||
               szFromFName[uiStrLen - 6] != 'h' ||
               szFromFName[uiStrLen - 5] != 'a' ||
               szFromFName[uiStrLen - 4] != 'n' ||
               szFromFName[uiStrLen - 3] != 'g' ||
               szFromFName[uiStrLen - 2] != 'e' ||
               szFromFName[uiStrLen - 1] != 'd')
            {
                fprintf(stderr,
                        "Could not find field named %s\n", 
                        szOutFieldname);
            }
            else
            {
                szFromFName[uiStrLen - 8] = '\0';

                DataElementDesc *pDesc = 
                    pFrom->getReflexType().findElementDesc(
                        szFromFName.c_str());
    ...
    mapIt = _pNameNodeMap->find(szToName);
    ...
    if(mapIt != _pNameNodeMap->end())
    {
        ...
        DataElementDesc *pDesc = 
            pTo->getReflexType().findElementDesc(szInFieldname);

        if(pDesc != NULL)
        {
            uiToId = pDesc->getElementId();
            uiValid++;
        }
        else
        {
            if(szInFieldname[0] != 's' ||
               szInFieldname[1] != 'e' ||
               szInFieldname[2] != 't' ||
               szInFieldname[3] != '_')
            {
            ...
            {
                DataElementDesc *pDesc = 
                    pTo->getReflexType().findElementDesc(&(szInFieldname[4]));
    ...
    if(uiValid == 4)
    {
        VRMLRouteTask::the()->addRoute(pFrom, uiFromId,
                                          pTo,   uiToId  );
    }

/*
    fprintf(stderr, "Add Route (%d) : %s.%s -> %s.%s\n%p.%d -> %p.%d\n",
            uiValid,
            szOutNodename,
            szOutFieldname,
            szInNodename,
